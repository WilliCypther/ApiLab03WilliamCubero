 ...
 static deleteAll = async (req: Request, resp: Response) => {
  try {
    ...
    if (!id) {
      ...
      return resp.status(404).json({ mensaje: "¡USUARIO! Debe indicar el ID de la factura a eliminar" });
    }
    ...
    if (!factura) {
      ...
      return resp.status(404).json({ mensaje: "¡ATENCION USUARIO! No se encuentra la factura con ese ID" });
    }

    // Verificar si la factura ya está eliminada lógicamente
    if (factura.eliminada) {
      return resp.status(400).json({ mensaje: "¡ATENCION USUARIO! La factura ya ha sido eliminada" });
    }
    ...
    return resp.status(200).json({ mensaje: "¡ATENCION USUARIO! La factura se eliminó correctamente" });
  } catch (error) {
    // Manejo de errores: devolver un mensaje de error
    return resp.status(400).json({ mensaje: "¡ATENCION USUARIO! No se pudo eliminar la factura" });
  }
};
...
static addAll = async (req: Request, resp: Response) => {
  try {
    ...
    if (!Numero) {
      return resp.status(404).json({ mensaje: "¡ATENCION USUARIO! Debe indicar el Numero" });
    }
    if (!Estado) {
      return resp.status(404).json({ mensaje: "¡ATENCION USUARIO! Debe indicar el estado" });
    }
    if (!Fecha) {
      return resp.status(404).json({ mensaje: " ¡ATENCION USUARIO! Debe indicar la fecha" });
    }
    if (!Ruc_Cliente) {
      return resp
        .status(404)
    ...
    if (!Codigo_vendedor) {
      return resp.status(404).json({ mensaje: "¡ATENCION USUARIO! Debe indicar el vendedor" });
    }
    if (Cantidad < 0) {
      return resp
        .status(404)
        .json({ mensaje: "¡ATENCION USUARIO! Debe indicar la Cantidad mayor que 0" });
    }
    if (!Codigo_producto) {
      return resp
        .status(404)
        .json({ mensaje: "¡ATENCION USUARIO! Debe indicar el Codigo del Productos" });
    }
    ...
    EsteFacturaDet = await ElDetalleRepo.findOne({ where: {  numero: Numero } });

    if (LaFacturaCab && EsteFacturaDet) {
      return resp
        .status(404)
        .json({ mensaje: "¡ATENCION USUARIO! La factura ya existe en la base de datos" });
    }
    ...
    try {
      ...
      return resp
        .status(404)
        .json({ mensaje: "¡ATENCION USUARIO! No se encontro el producto con ese ID" });
    }
    ...
    return resp.status(200).json({ mensaje: "¡ATENCION USUARIO! Producto Creado" });
  } catch (error) {
...
static update = async (req, resp) => {
  try {
    ...
    if (!numero) {
      return resp.status(404).json({ mensaje: "¡ATENCION USUARIO! se necesita indicar el ID" });
    }
    ...
    try {
      ...
      const { detalle_factura: [Codigo_producto, cantidad] } = req.body;

      // Buscar el producto a actualizar por su código
      const productoParaelUpdate = await detalle.findOneBy({
        Codigo_producto: Codigo_producto,
      });

      // Actualizar las propiedades del producto
      productoParaelUpdate.codigo_producto = Codigo_producto;
      productoParaelUpdate.cantidad = cantidad;

      // Guardar el producto actualizado en la base de datos
      await detalles.save(productoParaelUpdate);
      return resp.status(200).json({ mensaje: "¡ATENCION USUARIO! La Factura fue actualizada" });
    } catch (error) {
      return resp
        .status(404)
        .json({ mensaje: "¡ATENCION USUARIO! No se localizo la facutra con ese Id indicado" }); 
    }
  } catch (error) {
